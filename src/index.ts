...
import { attributeMemberJoin, attributeMemberLeave, primeInviteCache, syncGuildInvites } from "./services/invites.js";
...
client.on(Events.GuildMemberAdd, (member) => attributeMemberJoin(member, db).catch(console.error));
client.on(Events.GuildMemberRemove, (member) => attributeMemberLeave(member, db).catch(console.error));
...
client.on(Events.GuildCreate, (guild) => syncGuildInvites(guild, db).catch(console.error));
